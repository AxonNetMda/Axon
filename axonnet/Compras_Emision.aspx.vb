...
Public Class Compras_Emision
    ...
    Private Sub Compras_Emision_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            auxNoGravado = 0
            auxDescuento = 0
            Dim lisaproveedor As List(Of proveedor)
            Dim lisaresponsable As List(Of tipocomprobantes)
            nSuc = Session("IdSucursal")
            nUsuario = Session("Idusuario")
            lisaproveedor = New CN_Proveedor().Listar(0)
            For item = 0 To lisaproveedor.Count - 1
                cboProveedor.Items.Insert(item, New ListItem(lisaproveedor(item).RazonSocial, lisaproveedor(item).idProveedor))
            Next
            ...
            Dim listatipocompra As List(Of tipocompra) = New CD_TipoCompra().Listar()
            For item = 0 To listatipocompra.Count - 1
                cboTipoCompra.Items.Insert(item, New ListItem(listatipocompra(item).Nombre, listatipocompra(item).idTipocompra))
            Next
            ...
            Dim lisaproducto As List(Of producto) = New CD_Producto().Listar(0)
            ...
            If codigoProducto <> "0" Then
                ...
                End If
            End If
            ObtenerDetallesProducto(ddlProductos.SelectedValue)
    ...
    Protected Sub btnAgregarProducto_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAgregarProducto.Click
        If txtidproducto.Value = 0 Then
            Exit Sub
        End If
        Dim lista As List(Of producto) = New CN_Producto().Listar(ddlProductos.SelectedValue)
        If lista.Count > 0 Then
            ...
            Next
            If Not seguiR Then
                gvCompraDetalle.DataSource = dtDetalleFactura
    ...
    Private Function ObtenerDetallesProducto(codigoProducto As String) As DataTable
        ...
        Dim listaproducto As List(Of producto) = New CD_Producto().Listar(codigoProducto)
        If listaproducto.Count > 0 Then
            dt.Rows.Add(listaproducto(0).idProducto, listaproducto(0).Nombre, listaproducto(0).PrecioCosto, listaproducto(0).AlicuotaIVA)
        End If


        ' Simulación de datos (en una aplicación real, esta lógica estaría en una consulta SQL)
        'Select Case codigoProducto
        '    Case "001"
        '        dt.Rows.Add("001", 100.0, 21.0)
        '    Case "002"
        '        dt.Rows.Add("002", 200.0, 10.5)
        '    Case "003"
        '        dt.Rows.Add("003", 150.0, 21.0)
        'End Select
    ...
    Private Sub gvCompraDetalle_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvCompraDetalle.RowCommand
        If e.CommandName = "Anular" Then
            Dim index As Integer
            index = e.CommandArgument
            Dim dt As DataTable = CType(ViewState("DetalleFactura"), DataTable)
            For i = 0 To dt.Rows.Count - 1
                If dt.Rows(i).Item("IdProducto") = gvCompraDetalle.Rows(index).Cells(0).Text Then
                    dt.Rows(i).Delete()
                    Exit For
                End If
            Next i
        ...
        ElseIf e.CommandName = "editar" Then
            ...
            txtPrecioUnitario.Text = gvCompraDetalle.Rows(index).Cells(3).Text
            txtCantidad.Text = gvCompraDetalle.Rows(index).Cells(2).Text
            For Each item As ListItem In ddlProductos.Items

                ' Compara el valor de la celda con el valor del ítem del ComboBox.
                If item.Value = Val(gvCompraDetalle.Rows(index).Cells(0).Text) Then
    ...
    Private Sub TotalcomprobanteCargaitems()
        ...
        For i = 0 To gvCompraDetalle.Rows.Count - 1

            Precio = gvCompraDetalle.Rows(i).Cells(3).Text
            Cantidad = gvCompraDetalle.Rows(i).Cells(2).Text
    ...
    Private Sub GrabarComprobante()
        '    CREATE Type EDetalleCompra AS TABLE (
        '    idCompra Int,
        '    idProducto INT,
        '    Cantidad Int,
        ...
        Try
            ...
            Dim detalles As New List(Of compra_detalle)
            ...
            For i = 0 To gvCompraDetalle.Rows.Count - 1
                tbldetalles.Rows.Add(gvCompraDetalle.Rows(i).Cells(0).Text, gvCompraDetalle.Rows(i).Cells(2).Text, gvCompraDetalle.Rows(i).Cells(3).Text, gvCompraDetalle.Rows(i).Cells(4).Text, gvCompraDetalle.Rows(i).Cells(5).Text, referecia)
                detalles.Add(New compra_detalle With {
                    .idProducto = New producto() With {.idProducto = Convert.ToInt32(gvCompraDetalle.Rows(i).Cells(0).Text)},
                    .cantidad = Convert.ToInt32(gvCompraDetalle.Rows(i).Cells(2).Text),
                    .Precio = Convert.ToDecimal(gvCompraDetalle.Rows(i).Cells(3).Text),
                    .AlicuotaIVA = Convert.ToDecimal(gvCompraDetalle.Rows(i).Cells(4).Text),
                    .SubTotal = Convert.ToDecimal(gvCompraDetalle.Rows(i).Cells(5).Text),
            ...
            End If
            objCompra = New compra() With {
                .oSucursal = New sucursal() With {.idSucursal = Convert.ToInt32(Session("idSucursal"))},
                .oTipoCompra = New tipocompra() With {.idTipocompra = cboTipoCompra.SelectedValue},
                .fecha = Date.Today,
                ...
                .Estado = 1,
                .oProveedor = New proveedor() With {.idProveedor = cboProveedor.SelectedValue},
            ...
            idgenerado = New CD_Compra().Registrar(objCompra, tbldetalles, mensaje)
